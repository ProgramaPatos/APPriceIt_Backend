...
CREATE TABLE pricereview (
       ...
       pricereview_appuser_id int NOT NULL REFERENCES appuser (appuser_id) ON UPDATE CASCADE,
       pricereview_price_id int NOT NULL REFERENCES price (price_id) ON UPDATE CASCADE
);
...
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO appuser (appuser_name, appuser_password, appuser_creation_date, appuser_state)
...
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO price(price_value, price_timestamp, price_appuser_id)
...
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO appuserrole(appuserrole_role_id, appuserrole_appuser_id)
    VALUES (id_role, id_user);
END;

CREATE PROCEDURE assign_product_tag(
    id_product int,
    id_tag int
)
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO producttag(producttag_product_id, producttag_tag_id)
    VALUES (id_product, id_tag);
END;

CREATE PROCEDURE create_price_review(
    score int,
    id_price int,
    comment text = NULL,
    id_user int = NULL
...
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO pricereview(pricereview_score, pricereview_creation_timestamp, pricereview_modification_timestamp, 
       pricereview_comment, pricereview_appuser_id, pricereview_price_id)
    VALUES (score, NOW(), NOW(), comment, id_user, id_price);
END;

CREATE PROCEDURE assign_product_to_store(
    availability int,
    id_product int,
    id_store int,
    id_price int
...
LANGUAGE SQL
BEGIN ATOMIC
    INSERT INTO productatstore(productatstore_availability, productatstore_product_id, productatstore_store_id, productatstore_price_id)
