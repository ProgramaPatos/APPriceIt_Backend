...
export class StoreService {
  ...
  async findOneStore(id: number): Promise<StoreQueryDTO> {
    ...
    if (res.length == 0) {
      ...
      throw new UnprocessableEntityException(`Multiple stores with ${id} found`);
    }
    return res[0];
  }

  async createStore(newStore: StoreCreateDTO) {
    const _res = await this.pgdb.proc(
      "fun.create_store",
      [
  ...
  async searchStores({ lat, lon, distance, name_prefix }: StoreQueryDTO): Promise<StoreResponseDTO[]> {
    let res: StoreResponseDTO[];
    if (name_prefix) {
      res = await this.pgdb.func(
  ...
  async updateStore(store_id: number, { store_appuser_id, store_name, store_lat, store_lon, store_description, store_schedule }: StoreUpdateDTO) {
    const res = (await this.pgdb.func(
      "fun.update_store",
      [
        store_appuser_id,
    ...
    if (res === -1) {
      ...
      throw new ForbiddenException(
        `Store with id "${store_id}" was not created by user with id "${store_appuser_id}"`)
    }
  }
}
